...
Devmode-auto-auth fires **only** when the server sees no header. So
`-u admin:<wrong-guess>` returns `HTTP 401` even in devmode — the server
validates whatever you did send and rejects it. Don't try `-u admin:fusion`
or `-u admin:admin` "just in case"; pick the right form for the server's
mode and only fall back to a real password if the user supplies one.

### Sending an lsFusion script via curl

Use **POST** with `Content-Type: application/x-www-form-urlencoded; charset=UTF-8`
and pass the script through `--data-urlencode script=...`. The charset is
non-optional: drop it and Cyrillic / accented strings get mangled.
...
| `HTTP 200`, body present | Script declared an `EXPORT FROM ...` or used `&return=<var>` to send a value back. See "Getting values back" below. |
...
**1. `&return=<property name>` with a global property.** Define a property in
the script that holds the answer; pass its name as `return=`.

```bash
curl -sS -u 'admin:' -X POST \
  -H 'Content-Type: application/x-www-form-urlencoded; charset=UTF-8' \
  --data-urlencode "script=
itemCount () = GROUP SUM 1 IF Item i IS Item;
run() { }
" \
  --data-urlencode "return=itemCount" \
  http://localhost:7651/eval
```

Note this uses `/eval` (which expects a `run` action) rather than `/eval/action`.

**2. `EXPORT FROM` to stdout via the response body.** The action's `EXPORT`
target becomes the HTTP response body — content type is derived from the
...
curl -sS -u 'admin:' -X POST \
  ...
  --data-urlencode "script=
LOCAL csv = FILE();
EXPORT CSV FROM idItem = id(Item i), nameItem = name(i) WHERE i IS Item;
EXPORTFILE();
" \
  http://localhost:7651/eval/action
```
...
// Module loaded? Cheap check — reference a property only that module defines.
MESSAGE 'has Eval module = ' + STRING(GROUP SUM 1 IF evalLastTime() OR NOT evalLastTime());
```
...
separate eval call that reads the count back via `&return=` or
`MESSAGE`) or with a Playwright screenshot of the list form (Part 3).
...
the host's external port). Body is `application/json`. The app-server port
`7651` does not serve these — RMI only.
...
curl -sS -u 'admin:' -X POST \
  -H 'Content-Type: application/json' \
  -d '{"regex":"<propertyName>","pathPattern":"**/*.lsf","limit":50,"contextChars":80}' \
...
- **Charset matters.** Always send
  `Content-Type: application/x-www-form-urlencoded; charset=UTF-8`. Without
  the charset suffix, non-ASCII bytes in the script (Cyrillic, accents) end
  up as `?` server-side. The script then fails name resolution against a
  Cyrillic-named class with a misleading "not found" error.
